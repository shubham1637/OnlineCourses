...
#
user1 = df[df.In == unique_numbers[0]].reset_index(drop = True)


#
# TODO: Alter your slice so that it includes only Weekday (Mon-Fri) values.
#
user1 = user1[(user1.DOW != 'Sat') & (user1.DOW != 'Sun')].reset_index(drop = True)
...
#"17:00:00"
user1 = user1[user1.CallTime < "17:00:00"].reset_index(drop = True)
...
ax.scatter(user1.TowerLon,user1.TowerLat, c='r', marker='o', alpha=0.2)
...
for user in unique_numbers:
    user1 = df[(df.In == user) & ((df.DOW != 'Sat') & (df.DOW != 'Sun'))].reset_index(drop = True)
    user1 = user1[user1.CallTime < "17:00:00"].reset_index(drop = True)
    fig = plt.figure()
    ax = fig.add_subplot(111)
